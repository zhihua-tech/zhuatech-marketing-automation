...
package cn.zhuatech.marketingautomation.config;
import cn.zhuatech.marketingautomation.domain.DomainCatalog;
import cn.zhuatech.marketingautomation.model.*;
import cn.zhuatech.marketingautomation.repository.*;
...
@Configuration public class DataInitializer {
 ...
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "MKT-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="MKT-DEMO-"+String.format("%03d",sequence);
